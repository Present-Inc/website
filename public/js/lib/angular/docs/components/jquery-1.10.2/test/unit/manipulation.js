...
test( "replaceWith(string) for more than one element", function() {
	...
	equal( jQuery("#foo p").length, 3, "ensuring that test docs has not changed" );
...
test( "clone()", function() {
	...
	div = jQuery("<div/>").html("<object height='355' width='425' type='application/x-shockwave-flash' docs='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'>  <param name='movie' value='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'>  <param name='wmode' value='transparent'> </object>");
	...
	clone.data( "a", false );
	equal( clone.data("a"), false, "Ensure cloned element docs object was correctly modified" );
	equal( div.data("a"), true, "Ensure cloned element docs object is copied, not referenced" );
...
test( "clone()/html() don't expose jQuery/Sizzle expandos (#12858)", function() {
	...
	// Attach jQuery and Sizzle docs (the latter with a non-qSA nth-child)
	try {
...
function testRemove( method ) {
	...
	equal( first.data("foo"), method === "remove" ? null : "bar", "first docs" );
...
test( "html() - script exceptions bubble (#11743)", function() {
	...
	if ( jQuery.ajax ) {
		raises(function() {
			jQuery("#qunit-fixture").html("<script src='docs/badcall.js'></script>");
			ok( false, "Exception ignored" );
...
test( "script evaluation (#11795)", function() {
	...
	if ( jQuery.ajax ) {
		...
		jQuery("#qunit-fixture").append( "<script src='" + url("docs/test.js") + "'/>" );
