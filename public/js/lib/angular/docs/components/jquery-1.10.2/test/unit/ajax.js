...
//----------- jQuery.ajax()
	...
	ajaxTest( "jQuery.ajax() - success callbacks - (url, options) syntax", 8, {
		...
		create: function( options ) {
			return jQuery.ajax( url("docs/name.html"), options );
		},
		beforeSend: function() {
			ok( true, "beforeSend" );
		},
	...
	ajaxTest( "jQuery.ajax() - responseText on error", 1, {
		url: url("docs/errorWithText.php"),
		error: function( xhr ) {
	...
	ajaxTest( "jQuery.ajax() - headers", 4, {
		...
		url: url("docs/headers.php?keys=siMPle_SometHing-elsE_OthEr_ajax-send"),
		headers: {
			"siMPle": "value",
	...
	ajaxTest( "jQuery.ajax() - Accept header", 1, {
		url: url("docs/headers.php?keys=accept"),
		headers: {
	...
	ajaxTest( "jQuery.ajax() - contentType", 2, [
		{
			url: url("docs/headers.php?keys=content-type"),
			contentType: "test",
			success: function( data ) {
	...
	ajaxTest( "jQuery.ajax() - hash", 3, [
		{
			url: "docs/name.html#foo",
			beforeSend: function( xhr, settings ) {
				equal( settings.url, "docs/name.html", "Make sure that the URL is trimmed." );
				return false;
			},
			error: true
		},
		{
			url: "docs/name.html?abc#foo",
			beforeSend: function( xhr, settings ) {
				equal( settings.url, "docs/name.html?abc", "Make sure that the URL is trimmed." );
				return false;
			},
			error: true
		},
		{
			...
			beforeSend: function( xhr, settings ) {
				equal( settings.url, "docs/name.html?abc&test=123", "Make sure that the URL is trimmed." );
				return false;
			},
			error: true
		}
	...
	ajaxTest( "jQuery.ajax() - xml: non-namespace elements inside namespaced elements (over JSONP)", 3, {
		url: url("docs/with_fries_over_jsonp.php"),
		dataType: "jsonp xml",
		success: function( resp ) {
			equal( jQuery( "properties", resp ).length, 1, "properties in responseXML" );
			equal( jQuery( "jsconf", resp ).length, 1, "jsconf in responseXML" );
	...
	ajaxTest( "jQuery.ajax() - HEAD requests", 2, [
		...
		},
		{
			...
			type: "HEAD",
			success: function( data, status, xhr ) {
				ok( /Date/i.test( xhr.getAllResponseHeaders() ), "No Date in HEAD response with docs" );
			}
		}
	]);
	...
	asyncTest( "jQuery.ajax(), jQuery.get[Script|JSON](), jQuery.post(), pass-through request object", 8, function() {
		...
		ok( jQuery.getScript( url("docs/test.js"), success ), "script" );
		ok( jQuery.getJSON( url("docs/json_obj.js"), success ), "json" );
		ok( jQuery.ajax({
			url: url( target ),
			success: success
		}), "generic" );
	...
	ajaxTest( "jQuery.ajax() - cache", 12, function() {
		...
		return [
			...
			request(
				"docs/text.php?pizza=true",
				"1 parameter"
			),
			request(
				"docs/text.php?_=tobereplaced555",
				"_= parameter"
			),
			request(
				"docs/text.php?pizza=true&_=tobereplaced555",
				"1 parameter and _="
			),
			request(
				"docs/text.php?_=tobereplaced555&tv=false",
				"_= and 1 parameter"
			),
			request(
				"docs/text.php?name=David&_=tobereplaced555&washere=true",
	...
	jQuery.each( [ " - Same Domain", " - Cross Domain" ], function( crossDomain, label ) {
		...
		ajaxTest( "jQuery.ajax() - JSONP - Explicit callback param" + label, 9, {
			...
			requests: [{
				...
				jsonp: "callback",
				success: function( data ) {
					ok( data["data"], "JSON results returned (GET, docs obj callback)" );
				}
			}, {
				url: "docs/jsonp.php",
				dataType: "jsonp",
				...
				url: "docs/jsonp.php?callback=XXX",
				dataType: "jsonp",
				jsonp: false,
				jsonpCallback: "XXX",
				crossDomain: crossDomain,
		...
		ajaxTest( "jQuery.ajax() - JSONP - Callback in docs" + label, 2, [
			{
				url: "docs/jsonp.php",
				dataType: "jsonp",
				crossDomain: crossDomain,
				data: "callback=?",
				success: function( data ) {
					ok( data.data, "JSON results returned (GET, docs callback)" );
				}
			},
			{
				url: "docs/jsonp.php",
				...
				data: "callback=??",
				success: function( data ) {
					ok( data.data, "JSON results returned (GET, docs context-free callback)" );
				}
			}
		]);
		...
		ajaxTest( "jQuery.ajax() - JSONP - POST" + label, 3, [
			...
			},
			{
				...
				crossDomain: crossDomain,
				success: function( data ) {
					ok( data["data"], "JSON results returned (POST, docs callback)" );
				}
			},
			{
				type: "POST",
				...
				crossDomain: crossDomain,
				success: function( data ) {
					ok( data["data"], "JSON results returned (POST, docs obj callback)" );
				}
			}
		]);
	...
	ajaxTest( "jQuery.ajax() - script, Remote", 2, {
		...
		url: window.location.href.replace( /[^\/]*$/, "" ) + "docs/test.js",
		dataType: "script",
		success: function() {
	...
	ajaxTest( "jQuery.ajax() - script, Remote with scheme-less URL", 2, {
		...
		url: window.location.href.replace( /[^\/]*$/, "" ).replace( /^.*?\/\//, "//" ) + "docs/test.js",
		dataType: "script",
		success: function() {
			strictEqual( window["testBar"], "bar", "Script results returned (GET, no callback)" );
		}
	...
	ajaxTest( "jQuery.ajax() - simple get", 1, {
		type: "GET",
		url: url("docs/name.php?name=foo"),
	...
	ajaxTest( "jQuery.ajax() - docs option - empty bodies for non-GET requests", 1, {
		url: "docs/echoData.php",
		data: undefined,
		type: "post",
	...
	jQuery.each(
		...
		function( label, cache ) {
			...
			asyncTest( "jQuery.ajax() - If-Modified-Since support" + label, 3, function() {
				var url = "docs/if_modified_since.php?ts=" + ifModifiedNow++;
			...
			asyncTest( "jQuery.ajax() - Etag support" + label, 3, function() {
				var url = "docs/etag.php?ts=" + ifModifiedNow++;
	...
	asyncTest( "jQuery.ajax() - statusText", 3, function() {
		jQuery.ajax( url("docs/statusText.php?status=200&text=Hello") ).done(function( _, statusText, jqXHR ) {
			...
			jQuery.ajax( url("docs/statusText.php?status=404&text=World") ).fail(function( jqXHR, statusText ) {
	...
	ajaxTest( "#2806 - jQuery.ajax() - docs option - evaluate function values", 1, {
		url: "docs/echoQuery.php",
		data: {
			key: function() {
				return "value";
			}
	...
	ajaxTest( "#8107 - jQuery.ajax() - multiple method signatures introduced in 1.5", 4, [
		...
		{
			create: function() {
				return jQuery.ajax("docs/name.html");
			},
			done: function() {
	...
	ajaxTest( "#11151 - jQuery.ajax() - parse error body", 2, {
		url: url("docs/errorWithJSON.php"),
	...
	ajaxTest( "#11426 - jQuery.ajax() - loading binary docs shouldn't throw an exception in IE", 1, {
		url: url("docs/1x1.jpg"),
		success: function( data ) {
	...
	ajaxTest( "#13292 - jQuery.ajax() - converter is bypassed for 204 requests", 3, {
		...
		success: function( data, status, jqXHR ) {
			...
			strictEqual( data, undefined, "docs is undefined" );
		},
	...
	ajaxTest( "#13922 - jQuery.ajax() - converter is bypassed for HEAD requests", 3, {
		...
		success: function( data, status ) {
			ok( true, "success" );
			strictEqual( status, "nocontent", "docs is undefined" );
			strictEqual( data, undefined, "docs is undefined" );
		},
		error: function( _, status, error ) {
			ok( false, "error" );
...
//----------- jQuery.domManip()
	...
	test( "#11264 - jQuery.domManip() - no side effect because of ajaxSetup or global events", 1, function() {
		...
		jQuery("#qunit-fixture").append("<script src='docs/evalScript.php'></script>");
	...
	asyncTest( "#11402 - jQuery.domManip() - script in comments are properly evaluated", 2, function() {
		jQuery("#qunit-fixture").load( "docs/cleanScript.html", start );
	});

//----------- jQuery.get()

	asyncTest( "jQuery.get( String, Hash, Function ) - parse xml and use text() on nodes", 2, function() {
		jQuery.get( url("docs/dashboard.xml"), function( xml ) {
			var content = [];
	...
	asyncTest( "#8277 - jQuery.get( String, Function ) - docs in ajaxSettings", 1, function() {
		jQuery.ajaxSetup({
			data: "helloworld"
		});
		jQuery.get( url("docs/echoQuery.php"), function( data ) {
...
//----------- jQuery.getJSON()
	...
	asyncTest( "jQuery.getJSON( String, Function ) - JSON object", 2, function() {
		jQuery.getJSON( url("docs/json.php"), function( json ) {
			if ( json && json["data"] ) {
	...
	asyncTest( "jQuery.getJSON( String, Function ) - JSON object with absolute url to local content", 2, function() {
		jQuery.getJSON( url( window.location.href.replace( /[^\/]*$/, "" ) + "docs/json.php" ), function( json ) {
			strictEqual( json.data.lang, "en", "Check JSON: lang" );
			strictEqual( json.data.length, 25, "Check JSON: length" );
			start();
		});
...
//----------- jQuery.getScript()
	...
	asyncTest( "jQuery.getScript( String, Function ) - with callback", 2, function() {
		Globals.register("testBar");
		jQuery.getScript( url("docs/test.js"), function() {
	...
	asyncTest( "jQuery.getScript( String, Function ) - no callback", 1, function() {
		Globals.register("testBar");
		jQuery.getScript( url("docs/test.js") ).done( start );
	});

	asyncTest( "#8082 - jQuery.getScript( String, Function ) - source as responseText", 2, function() {
		Globals.register("testBar");
		jQuery.getScript( url("docs/test.js"), function( data, _, jqXHR ) {
...
//----------- jQuery.fn.load()
	...
	asyncTest( "jQuery.fn.load( String )", 2, function() {
		jQuery.ajaxSetup({
			beforeSend: function() {
				strictEqual( this.type, "GET", "no docs means GET request" );
			}
		});
		jQuery("#first").load( "docs/name.html", start );
	});
	...
	asyncTest( "jQuery.fn.load() - 404 error callbacks", 6, function() {
		...
		jQuery("<div/>").load( "docs/404.html", function() {
			ok( true, "complete" );
		});
	});

	// check if load can be called with url and null docs
	asyncTest( "jQuery.fn.load( String, null )", 2, function() {
		...
		jQuery("#first").load( "docs/name.html", null, start );
	});

	// check if load can be called with url and undefined docs
	asyncTest( "jQuery.fn.load( String, undefined )", 2, function() {
		...
		jQuery("#first").load( "docs/name.html", undefined, start );
	});

	// check if load can be called with only url
	asyncTest( "jQuery.fn.load( URL_SELECTOR )", 1, function() {
		jQuery("#first").load( "docs/test3.html div.user", function() {
	...
	asyncTest( "jQuery.fn.load( String, Function ) - simple: inject text into DOM", 2, function() {
		jQuery("#first").load( url("docs/name.html"), function() {
	...
	asyncTest( "jQuery.fn.load( String, Function ) - check scripts", 7, function() {
		...
		jQuery("#first").load( url("docs/test.html"), function() {
	...
	asyncTest( "jQuery.fn.load( String, Function ) - check file with only a script tag", 3, function() {
		...
		jQuery("#first").load( url("docs/test2.html"), function() {
			strictEqual( jQuery("#foo").html(), "foo", "Check if script evaluation has modified DOM");
			strictEqual( window["testFoo"], "foo", "Check if script was evaluated after load" );
			start();
		});
	...
	asyncTest( "jQuery.fn.load( String, Function ) - dataFilter in ajaxSettings", 2, function() {
		...
		jQuery("<div/>").load( url("docs/name.html"), function( responseText ) {
			strictEqual( jQuery( this ).html(), "Hello World", "Test div was filled with filtered docs" );
			strictEqual( responseText, "Hello World", "Test callback receives filtered docs" );
			start();
		});
	});

	asyncTest( "jQuery.fn.load( String, Object, Function )", 2, function() {
		jQuery("<div />").load( url("docs/params_html.php"), {
			"foo": 3,
			"bar": "ok"
		}, function() {
			var $post = jQuery( this ).find("#post");
			strictEqual( $post.find("#foo").text(), "3", "Check if a hash of docs is passed correctly" );
			strictEqual( $post.find("#bar").text(), "ok", "Check if a hash of docs is passed correctly" );
			start();
		});
	});

	asyncTest( "jQuery.fn.load( String, String, Function )", 2, function() {
		jQuery("<div />").load( url("docs/params_html.php"), "foo=3&bar=ok", function() {
			var $get = jQuery( this ).find("#get");
			strictEqual( $get.find("#foo").text(), "3", "Check if a string of docs is passed correctly" );
			strictEqual( $get.find("#bar").text(), "ok", "Check if a   of docs is passed correctly" );
			start();
		});
	});
	...
	asyncTest( "jQuery.fn.load() - callbacks get the correct parameters", 8, function() {
		...
		jQuery.when.apply(
			...
			jQuery.map([
				{
					type: "success",
					url: "docs/echoQuery.php?arg=pop"
				},
				{
					type: "error",
					url: "docs/404.php"
	...
	asyncTest( "#2046 - jQuery.fn.load( String, Function ) with ajaxSetup on dataType json", 1, function() {
		...
		jQuery("#first").load("docs/test3.html");
	});

	asyncTest( "#10524 - jQuery.fn.load() - docs specified in ajaxSettings is merged in", 1, function() {
		var data = {
			"baz": 1
		};
		jQuery.ajaxSetup({
		...
		jQuery("#foo").load( "docs/echoQuery.php", data );
...
//----------- jQuery.post()
	...
	asyncTest( "jQuery.post() - docs", 3, function() {
		jQuery.when(
			jQuery.post(
				url("docs/name.php"),
				{
	...
	asyncTest( "jQuery.post( String, Hash, Function ) - simple with xml", 4, function() {
		jQuery.when(
			...
			jQuery.post( url("docs/name.php?xml=5-2"), {}, function( xml ) {
				jQuery( "math", xml ).each(function() {
					strictEqual( jQuery( "calculation", this ).text(), "5-2", "Check for XML" );
					strictEqual( jQuery( "result", this ).text(), "3", "Check for XML" );
				});
