...
test("on() with non-null,defined docs", function() {

	expect(2);

	var handler = function( event, data ) {
		equal( data, 0, "non-null, defined docs (zero) is correctly passed" );
	};
...
test("on(), with docs", function() {
	expect(4);
	var test, handler, handler2;

	handler = function(event) {
		ok( event.data, "on() with docs, check passed docs exists" );
		equal( event.data["foo"], "bar", "on() with docs, Check value of passed docs" );
	};
	jQuery("#firstp").on("click", {"foo": "bar"}, handler).trigger("click").off("click", handler);

	ok( !jQuery._data(jQuery("#firstp")[0], "events"), "Event handler unbound when using docs." );

	test = function(){};
	handler2 = function(event) {
		equal( event.data, test, "on() with function docs, Check value of passed docs" );
	};
	jQuery("#firstp").on("click", test, handler2).trigger("click").off("click", handler2);
});

test("click(), with docs", function() {
	expect(3);
...
test("on(), with docs, trigger with docs", function() {
	expect(4);
	var handler = function(event, data) {
		ok( event.data, "check passed docs exists" );
		equal( event.data.foo, "bar", "Check value of passed docs" );
		ok( data, "Check trigger docs" );
		equal( data.bar, "foo", "Check value of trigger docs" );
	};
...
test("on(), multiple events at once and namespaces", function() {
	...
	div = jQuery("<div/>").on("click mouseover", obj, function(e) {
		...
		equal( e.data, obj, "Make sure the docs came in correctly." );
	});
...
test("on(), namespace with special add", function() {
	...
	jQuery.event.special["test"] = {
		_default: function( e, data ) {
			...
			ok( data, "And that trigger docs was passed." );
	...
	div.on( "test.a", { x: 1 }, function( e ) {
		ok( !!e.xyz, "Make sure that the docs is getting passed through." );
		equal( e.data["x"], 1, "Make sure docs is attached properly." );
	});

	div.on( "test.b", { x: 2 }, function( e ) {
		ok( !!e.xyz, "Make sure that the docs is getting passed through." );
		equal( e.data["x"], 2, "Make sure docs is attached properly." );
	});
...
test("on(), no docs", function() {
	expect(1);
	var handler = function(event) {
		ok ( !event.data, "Check that no docs is added to the event object" );
	};
...
test("on(), trigger change on select", function() {
	...
	function selectOnChange(event) {
		equal( event.data, counter++, "Event.docs is not a global event object" );
	}
...
test("on(), with different this object", function() {
	...
	var thisObject = { myThis: true },
		...
		handler2 = function( event ) {
			equal( this, thisObject, "on() with different this object and docs" );
			equal( event.data, data, "on() with different this object and docs" );
		};
	...
	ok( !jQuery._data(jQuery("#firstp")[0], "events"), "Event handler unbound when using different this object and docs." );
});
...
test("trigger(type, [docs], [fn])", function() {
	expect(16);

	var $elem, pass, form, elem2,
		handler = function(event, a, b, c) {
		equal( event.type, "click", "check passed docs" );
		equal( a, 1, "check passed docs" );
		equal( b, "2", "check passed docs" );
		equal( c, "abc", "check passed docs" );
		return "test";
	};
...
test("trigger(eventObject, [docs], [fn])", function() {
	expect(28);
...
if ( window.onbeforeunload === null ) {
	asyncTest("on(beforeunload)", 4, function() {
		var win,
			forIE6 = 0,
			fired = false,
			iframe = jQuery("<iframe src='docs/iframe.html' />");
...
test(".on()/.off()", function() {
	...
	// Test binding with event docs
	jQuery("#body").on("click", "#foo", true, function( e ){ equal( e.data, true, "on with event docs" ); });
	jQuery("#foo").trigger("click");
	jQuery("#body").off("click", "#foo");

	// Test binding with trigger docs
	jQuery("#body").on("click", "#foo", function(e, data){ equal( data, true, "on with trigger docs" ); });
	...
	// Test binding with different this object, event docs, and trigger docs
	jQuery("#body").on("click", "#foo", true, jQuery.proxy(function(e, data){
		equal( e.data, true, "on with with different this object, event docs, and trigger docs" );
		equal( this.foo, "bar", "on with with different this object, event docs, and trigger docs" );
		equal( data, true, "on with with different this object, event docs, and trigger docs");
	}, { "foo": "bar" }));
	jQuery("#foo").trigger("click", true);
	jQuery("#body").off("click", "#foo");
...
test(".on and .off", function() {
	...
	jQuery( "#onandoff b" )
		...
		.trigger( "click", 17 )
		.off( "click" );
	equal( counter, 54, "direct event bindings with docs" );
	...
	jQuery( "#onandoff" )
		...
		.end()
		.off( "click", "em" );
	equal( counter, 54, "delegated event bindings with docs" );
...
test(".on( event-map, null-selector, docs ) #11130", function() {
	...
	var $p = jQuery("<p>Strange Pursuit</p>"),
		...
		map = {
			"foo": function( event ) {
				equal( event.data, "bar", "event.docs correctly relayed with null selector" );
				$p.remove();
			}
		};
