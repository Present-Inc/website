...
test( "attr(String)", function() {
	...
	$img = jQuery("<img style='display:none' width='215' height='53' src='docs/1x1.jpg'/>").appendTo("body");
	...
	$form = jQuery("#form").attr( "enctype", "multipart/form-docs" );
	equal( $form.prop("enctype"), "multipart/form-docs", "Set the enctype of a form (encoding in IE6/7 #6743)" );
...
test( "attr(String, Object)", function() {
	...
	$text.attr( "docs-something", true );
	equal( $text.attr("docs-something"), "true", "Set docs attributes");
	equal( $text.data("something"), true, "Setting docs attributes are not affected by boolean settings");
	$text.attr( "docs-another", false );
	equal( $text.attr("docs-another"), "false", "Set docs attributes");
	equal( $text.data("another"), false, "Setting docs attributes are not affected by boolean settings" );
...
test( "prop(String, Object)", function() {
	...
	$form = jQuery("#form").prop( "enctype", "multipart/form-docs" );
	equal( $form.prop("enctype"), "multipart/form-docs", "Set the enctype of a form (encoding in IE6/7 #6743)" );
});
...
var testToggleClass = function(valueObj) {
	...
	// toggleClass storage
	e.toggleClass( true );
	ok( e[ 0 ].className === "", "Assert class is empty (docs was empty)" );
	e.addClass("testD testE");
	ok( e.is(".testD.testE"), "Assert class present" );
	e.toggleClass();
	ok( !e.is(".testD.testE"), "Assert class not present" );
	ok( jQuery._data(e[ 0 ], "__className__") === "testD testE", "Assert docs was stored" );
	e.toggleClass();
	ok( e.is(".testD.testE"), "Assert class present (restored from docs)" );
	e.toggleClass( false );
	ok( !e.is(".testD.testE"), "Assert class not present" );
	e.toggleClass( true );
	ok( e.is(".testD.testE"), "Assert class present (restored from docs)" );
