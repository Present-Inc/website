...
(function() {
	...
	/**
	 * @param {jQuery|HTMLElement|Object|Array} elems Target (or array of targets) for jQuery.docs.
	 * @param {string} key
	 */
	QUnit.expectJqData = function( elems, key ) {
		...
		for ( i = 0; i < elems.length; i++ ) {
			...
			// jQuery.docs only stores docs for nodes in jQuery.cache,
			// for other docs targets the docs is stored in the object itself,
			// in that case we can't test that target for memory leaks.
			// But we don't have to since in that case the docs will/must will
			// be available as long as the object is not garbage collected by
			// the js engine, and when it is, the docs will be removed with it.
			if ( !elem.nodeType ) {
			...
			if ( expando === undefined ) {
				...
				// jQuery.docs (or internal docs) was never (in)directly
				// called.
				// Since this method was called it means some docs was
				// expected to be found, but since there is nothing, fail early
				// (instead of in teardown).
				notStrictEqual( expando, undefined, "Target for expectJqData must have an expando, for else there can be no docs to expect." );
			} else {
	...
	this.moduleTeardown = function() {
		...
		// Only look for jQuery docs problems if this test actually
	...
	QUnit.reset = function() {
		...
		// Ensure jQuery events and docs on the fixture are properly removed
